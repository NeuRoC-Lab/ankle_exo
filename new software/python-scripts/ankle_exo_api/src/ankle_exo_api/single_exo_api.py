...
class SingleExoskeleton:
        ...
        try: # Zero all MIT commands
            ...
            while True:
                try:
                    ...
                    self.bluetooth.sd_queue.get_nowait()
                except Empty:
                    break

            print("Flushed sd, command and control queues")
            self.reset_command()
            print("Stopped motor")
            self.stop_motor()
    ...
    def start_recording(self):
        self._check_connection()
        self.bluetooth.queue_sd_control(SDControl(LoggingState.RECORDING))

    def stop_recording(self):
        self._check_connection()
        self.bluetooth.queue_sd_control(SDControl(LoggingState.STOPPED))
