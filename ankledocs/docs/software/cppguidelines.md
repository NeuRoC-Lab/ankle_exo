...
1) Reference the four load cells by their hardware pins. An abstraction was used in `Board.h` with a `enum` called `LoadCellId` which maps the position of the load cells (`Left1`,`Right2`, etc) to their hardware pins so the programer doesn't have to deal with low level pinouts
...
[F.60](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#f60-prefer-t-over-t-when-no-argument-is-a-valid-option): Prefer T* over T& when “no argument” is a valid option
